...
export function MarkdownDocument({ content, skipFirstHeading = false }: MarkdownDocumentProps) {
  const blocks = parseMarkdown(content);
  const visibleBlocks =
    skipFirstHeading && blocks[0]?.type === "heading" ? blocks.slice(1) : blocks;

  return (
    <div className="space-y-6 text-base/7 text-ink">
...
function renderBlock(block: MarkdownBlock, index: number) {
  if (block.type === "heading") {
    const className =
      block.level === 1
        ? "font-display text-3xl font-extrabold text-primary-700"
        : "pt-4 font-display text-2xl font-extrabold text-primary-700";
  ...
  if (block.type === "table") {
    return (
      <div key={index} className="overflow-x-auto rounded-3xl border border-primary-100">
        <table className="min-w-full divide-y divide-primary-100 bg-white text-left text-sm">
